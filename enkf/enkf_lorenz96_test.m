function[BIAS,MAE,RMSE]=enkf_lorenz96_test(n,N,ipv,r,nac,reps,ts)
...
%   ts  :   number of time steps between assimilations
    ...
    sc_t = {@(x) x, @(x) Q*x, @(x) n^(-.5)*fft(x),...
            @(x) transf1d(x,@(y)FWT_PO(y,L,qmf))};
    sc_it = {@(x) x, @(x) Q'*x, @(x) n^(.5)*ifft(x), ...
             @(x) transf1d(x,@(y) IWT_PO(y,L,qmf))};
    % wegihted innovation scenarios
    sc_w = {@(x,y) enkf_winov_sample(x,y,r),@(x,y) enkf_winov_diag(x,y,r),...
             @(x,y) enkf_winov_diag(x,y,r),@(x,y) enkf_winov_diag(x,y,r)};
    % update scenarios
    sc_u= {@(x,y,z) enkf_update_sample(x,y,z),@(x,y,z) enkf_update_diag(x,y,z),...
            @(x,y,z) enkf_update_diag(x,y,z),@(x,y,z) enkf_update_diag(x,y,z)};
    % names of scenarios
    sc_names = {'Sample','DST','FFT','DWT'};
    ...
    for rep_ind = 1:reps
        ...
        for sc_ind = 1:length(sc_names)
            [Y,X]=enkf_lorenz(Yi,Xens,r,ts,nac,l_f,sc_t{sc_ind},sc_it{sc_ind},sc_w{sc_ind},sc_u{sc_ind});
            XA = squeeze(mean(X,2));
            R = Y - XA;
    ...
    BIAS = mean(bias,3);
    figure()
    plot(PX,BIAS');title(sprintf('%s %.f','BIAS',N));
    legend(sc_names);
    xlabel('assimilation steps');
    RMSE = mean(rmse,3);
    figure();
    plot(PX,RMSE');title(sprintf('%s %.f','RMSE',N));
    legend(sc_names);
    xlabel('assimilation steps');
    MAE = mean(bias,3);
    figure();
    plot(PX,MAE');title(sprintf('%s %.f','MAE ',N));
    xlabel('assimilation steps');
    legend(sc_names);
end