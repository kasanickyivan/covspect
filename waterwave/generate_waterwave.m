...
%   dw_min,dw_max,dh_min,dh_max, bw_min - argumenst to add_rand_drop
...
function Y=generate_waterwave(n,reps,h,dw_min,dw_max,dh_min,dh_max,bdd,ts,init_ts,init_d,dper)
    ...
    for init_ind = 1:init_d
        Y(:,:,1,1)=add_rand_drop(squeeze(Y(:,:,1,1)),dw_min,dw_max,dh_min,dh_max,bdd);
    ...
    for rep_ind = 2:reps
        ...
        if(mod(rep_ind,dper)==0)
            Y(:,:,1,rep_ind)=add_rand_drop(squeeze(Y(:,:,1,rep_ind)),dw_min,dw_max,dh_min,dh_max,bdd);
